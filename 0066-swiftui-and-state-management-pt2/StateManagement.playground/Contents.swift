...
//ex) api한테 1000,000번째 소수를 달라고 얻으려는 예시
...
struct CounterView: View {
  ...
  var body: some View {
    VStack {
      ...
      .disabled(self.isNthPrimeButtonDisabled) // disable하지 않으면 아예 누르지도 못하게 하는 코드(API 너무 많이 부르지 않게 하기 위함)
    }
    .font(.title)
...
struct FavoritePrimesView: View {
  ...
  var body: some View {
    List {
      ForEach(self.state.favoritePrimes, id: \.self) { prime in // forEach 대신 map을 쓸 수 없음
        Text("\(prime)")
      }
